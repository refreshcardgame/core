import type { RCGCoordinate, RCGDirection } from "./types/orientation.ts";
...
export class RCGGrid implements RCGGridData {
  ...
  isInside(coordinate: RCGCoordinate, boundaryIndex?: number): boolean {
    if (typeof boundaryIndex !== "number")
      return this.boundaries.some((_, index) =>
        this.isInside(coordinate, index),
      );
